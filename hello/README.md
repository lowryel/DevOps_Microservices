...
```bash
hello$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
hello$ sam local start-api
hello$ curl http://localhost:3000/
```
...
```bash
hello$ sam logs -n HelloWorldFunction --stack-name hello --tail
```
...
```bash
hello$ pip install pytest pytest-mock --user
hello$ python -m pytest tests/ -v
```
